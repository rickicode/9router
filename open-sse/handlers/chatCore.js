...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, sourceFormatOverride, providerThinking }) {
  ...
  const bypassResponse = await handleBypassRequest(body, model, userAgent, ccFilterNaming);
  ...
  if (passthrough) {
    ...
    translatedBody = await translateRequest(sourceFormat, targetFormat, model, body, stream, credentials, provider, reqLogger, stripList, connectionId);
    if (!translatedBody) {
