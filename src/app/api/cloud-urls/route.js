  ...
  try {
    ...
    const updated = await writeCloudUrls((cloudUrls) => {
      ...
      const nextEntry = {
        ...
        url,
        isPrimary: isPrimary || cloudUrls.length === 0,
        status: VALID_STATUSES.has(body?.status) ? body.status : "unknown",
      };

      const nextUrls = [...cloudUrls, nextEntry];
      if (nextEntry.isPrimary) {
        return nextUrls.map((entry) => ({ ...entry, isPrimary: entry.id === nextEntry.id }));
      }
      return nextUrls;
    });
...
export async function PATCH(request) {
  try {
    ...
    const updated = await writeCloudUrls((cloudUrls) => {
      ...
      const nextUrls = cloudUrls.map((entry) => ({ ...entry }));
      const target = nextUrls[index];

      if (body.url !== undefined) {
        const url = normalizeUrl(body.url);
        if (!url) throw new Error("URL is required");
        if (!isValidHttpUrl(url)) throw new Error("URL must be a valid HTTP or HTTPS address");
        if (nextUrls.some((entry) => entry.id !== id && normalizeUrl(entry.url) === url)) {
          throw new Error("Cloud URL already exists");
        }
        target.url = url;
      }
      ...
      if (body.isPrimary === true) {
        nextUrls.forEach((entry) => {
          entry.isPrimary = entry.id === id;
        });
      } else if (body.isPrimary === false && target.isPrimary && nextUrls.length === 1) {
        target.isPrimary = true;
      }

      if (!nextUrls.some((entry) => entry.isPrimary)) {
        nextUrls[0].isPrimary = true;
      }
    ...
    const statusMap = {
      ...
      "URL is required": 400,
      "URL must be a valid HTTP or HTTPS address": 400,
      "Cloud URL already exists": 409,
      "Invalid status value": 400,
    };
    return NextResponse.json({ error: error.message || "Failed to update cloud URL" }, { status: statusMap[error.message] || 500 });
  }
}
...
export async function DELETE(request) {
  try {
    ...
    const updated = await writeCloudUrls((cloudUrls) => {
      ...
      const removed = cloudUrls[index];
      const nextUrls = cloudUrls.filter((entry) => Number(entry.id) !== id);
      if (removed.isPrimary && nextUrls.length > 0) {
        nextUrls[0] = { ...nextUrls[0], isPrimary: true };
        for (let i = 1; i < nextUrls.length; i += 1) {
          nextUrls[i] = { ...nextUrls[i], isPrimary: false };
        }
      }
      return nextUrls;
    });
