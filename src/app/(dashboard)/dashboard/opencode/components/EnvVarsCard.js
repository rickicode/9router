      ...
      <div className="space-y-6">
        <div className="rounded-[24px] border border-primary/10 bg-gradient-to-br from-primary/[0.05] via-transparent to-transparent px-5 py-[1.125rem]">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[13px] font-medium text-[var(--color-text-main)]">Config-backed variables</p>
              <p className="text-[11px] leading-5 text-[var(--color-text-muted)]">Secrets remain masked in the UI and should be re-entered before saving changes.</p>
            </div>
            <div className="rounded border border-primary/15 bg-[var(--color-primary)]/8 px-3 py-1 text-[11px] font-medium text-[var(--color-accent)]">
        ...
        <div className="space-y-4">
          {draftVars.length === 0 ? (
            <div className="rounded-[22px] border border-dashed border-black/8 bg-black/[0.015] px-5 py-6 text-[13px] text-[var(--color-text-muted)] dark:border-[var(--color-border)] dark:bg-[var(--color-surface)]/[0.015]">No environment variables configured yet.</div>
          ) : (
            draftVars.map((item, index) => (
              <Card.Section key={`${item.key || "env"}-${index}`} className="space-y-5 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-surface)]/[0.75] px-5 py-5 dark:border-white/5 dark:bg-[var(--color-surface)]/[0.02]">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[13px] font-medium text-[var(--color-text-main)]">Variable {index + 1}</p>
                    <p className="mt-1 text-[11px] leading-5 text-[var(--color-text-muted)]">{item.secret ? "Stored as a secret" : "Stored as plain text in preview"}</p>
                  </div>
