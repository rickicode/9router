...
function CooldownTimer({ until }) {
  ...
  return <span className="text-[11px] text-[var(--color-warning)] font-mono">⏱ {remaining}</span>;
}
...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  ...
  return (
    <div className={`group flex items-center justify-between p2 rounded hover:bg-black/[0.02] dark:hover:bg-[var(--color-surface)]/[0.02] transition-colors ${connection.isActive === false ? "opacity-60" : ""}`}>
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className="flex flex-col">
          <button onClick={onMoveUp} disabled={isFirst} className={`p-0.5 rounded ${isFirst ? "text-[var(--color-text-muted)]/30 cursor-not-allowed" : "hover:bg-[var(--color-primary)] text-[var(--color-text-muted)] hover:text-[var(--color-accent)]"}`}>
            <span className="material-symbols-outlined text-[13px]">keyboard_arrow_up</span>
          </button>
          <button onClick={onMoveDown} disabled={isLast} className={`p-0.5 rounded ${isLast ? "text-[var(--color-text-muted)]/30 cursor-not-allowed" : "hover:bg-[var(--color-primary)] text-[var(--color-text-muted)] hover:text-[var(--color-accent)]"}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {connection.lastError && connection.isActive !== false && (
              <span className="text-[11px] text-[var(--color-danger)] truncate max-w-[300px]" title={connection.lastError}>{connection.lastError}</span>
            )}
          ...
          {hasAnyProxy && (
            <div className="mt-1 flex items-center gap-2 flex-wrap">
              ...
              {maskedProxyUrl && <code className="text-[10px] font-mono bg-[rgba(0,0,0,0.05)] dark:bg-[var(--color-surface)] px-1 py-0.5 rounded text-[var(--color-text-muted)]">{maskedProxyUrl}</code>}
            ...
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-[var(--color-bg-alt)] border border-[var(--color-border)] rounded  py-1 min-w-[160px]">
                  <button onClick={() => handleSelectProxy("__none__")} className={`w-full text-left px-3 py-1.5 text-[13px] hover:bg-[rgba(0,0,0,0.05)] dark:hover:bg-[var(--color-surface)] ${!boundProxyPoolId ? "text-[var(--color-accent)] font-medium" : "text-[var(--color-text-main)]"}`}>None</button>
                  {(proxyPools || []).map((pool) => (
                    <button key={pool.id} onClick={() => handleSelectProxy(pool.id)} className={`w-full text-left px-3 py-1.5 text-[13px] hover:bg-[rgba(0,0,0,0.05)] dark:hover:bg-[var(--color-surface)] ${boundProxyPoolId === pool.id ? "text-[var(--color-accent)] font-medium" : "text-[var(--color-text-main)]"}`}>{pool.name}</button>
                  ))}
                </div>
              )}
            </div>
    ...
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <input className="w-full px-3 py-2 text-[13px] border border-[var(--color-border)] rounded bg-background focus:outline-none focus:border-primary" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Production Key" />
        </div>
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-[11px] text-[var(--color-text-muted)] mb-1 block">API Key</label>
            <input type="password" className="w-full px-3 py-2 text-[13px] border border-[var(--color-border)] rounded bg-background focus:outline-none focus:border-primary" value={formData.apiKey} onChange={(e) => setFormData({ ...formData, apiKey: e.target.value })} />
          </div>
          <div className="pt-6">
        ...
        <div>
          <label className="text-[11px] text-[var(--color-text-muted)] mb-1 block">Priority</label>
          <input type="number" className="w-full px-3 py-2 text-[13px] border border-[var(--color-border)] rounded bg-background focus:outline-none focus:border-primary" value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: Number.parseInt(e.target.value) || 1 })} />
        </div>
...
export default function ConnectionsCard({ providerId, isOAuth }) {
  ...
  if (loading) return <Card><div className="h-20 animate-pulse bg-[rgba(0,0,0,0.05)] rounded" /></Card>;
      ...
      <Card>
        ...
        {connections.length === 0 ? (
          <div className="flex items-center justify-between">
            <p className="text-[13px] text-[var(--color-text-muted)]">No connections yet</p>
