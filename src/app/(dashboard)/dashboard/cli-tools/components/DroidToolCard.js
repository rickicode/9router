      ...
      <div className="flex items-center justify-between hover:cursor-pointer" onClick={onToggle}>
        <div className="flex items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image src="/providers/droid.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} />
          </div>
              ...
              <div className="flex flex-col gap-3 p-4 bg-[var(--color-warning)]/10 border border-[var(--color-warning)]/30 rounded">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--color-warning)] ">Factory Droid CLI not detected locally</p>
                ...
                <div className="flex items-center gap-2 pl-9">
                  <Button variant="secondary" size="sm" onClick={() => setShowManualConfigModal(true)} className="!bg-[var(--color-warning)]/20 !border-[var(--color-warning)]/40 !text-[var(--color-warning)]  hover:!bg-[var(--color-warning)]/30">
                ...
                <div className="p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded">
                  ...
                  <div className="space-y-3 text-[13px]">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-[rgba(0,0,0,0.05)] dark:bg-[var(--color-surface)] rounded font-mono text-[11px]">curl -fsSL https://app.factory.ai/cli | sh</code>
                    </div>
                    <p className="text-[var(--color-text-muted)]">After installation, run <code className="px-1 bg-[rgba(0,0,0,0.05)] dark:bg-[var(--color-surface)] rounded">droid</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Models */}
                <div className="flex items-center gap-2">
                  <span className="w-32 shrink-0 text-[13px] font-medium text-[var(--color-text-main)] text-right">
                    Models {modelList?.length > 0 && <span className="text-[var(--color-accent)]">({modelList?.length})</span>}
                  </span>
                  <span className="material-symbols-outlined text-[var(--color-text-muted)] text-[14px]">arrow_forward</span>
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    {modelList?.length > 0 && (
                      <div className="flex flex-col gap-0.5 mb-1">
                        {modelList.map((id) => (
                          <div key={id} className="flex items-center gap-1.5 px-2 py-1 bg-[var(--color-bg-alt)]-secondary rounded border border-[var(--color-border)]">
                            <span className="flex-1 text-[11px] font-mono truncate">{id}</span>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`px-2 py-1.5 rounded border text-[11px] shrink-0 ${hasActiveProviders ? "bg-[var(--color-surface)] border-[var(--color-border)] hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-[var(--color-border)]"}`}
                      >
                        Select
                      </button>
                      <button onClick={addModel} disabled={!modelInput.trim()} className="px-2 py-1.5 rounded border bg-[var(--color-surface)] border-[var(--color-border)] hover:border-primary text-[11px] shrink-0 disabled:opacity-50" title="Add model">
