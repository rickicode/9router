...
export default function QuotaTable({ quotas = [], compact = false }) {
  ...
  const resetPrimary = compact ? "text-[11px]" : "text-[13px]";
        ...
        <tbody>
          {quotas.map((quota, index) => {
            ...
            return (
              <tr 
                key={index}
                className="border-b border-[var(--color-border)] dark:border-white/5 hover:bg-black/[0.02] dark:hover:bg-[var(--color-surface)]/[0.02] transition-colors"
              >
                {/* Model Name with Status Emoji */}
                <td className={`${cellPad} w-[30%]`}>
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className="text-[10px] shrink-0">{colors.emoji}</span>
                    <span className={`${nameText} font-medium text-[var(--color-accent)] truncate`}>
                      {quota.name}
                    </span>
                  </div>
                </td>
                ...
                <td className={`${cellPad} w-[45%]`}>
                  <div className={compact ? "space-y-1" : "space-y-1.5"}>
                    ...
                    <div className={`${compact ? "h-1" : "h-1.5"} rounded overflow-hidden border ${colors.bgLight} ${
                      remaining === 0 ? 'border-[var(--color-border)] dark:border-[var(--color-border)]' : 'border-transparent'
                    }`}>
                      <div
                    ...
                    {/* Numbers */}
                    <div className={`flex items-center justify-between ${compact ? "text-[10px]" : "text-[11px]"}`}>
                ...
                <td className={`${cellPad} w-[25%]`}>
                  {countdown !== "-" || resetDisplay ? (
                    <div className="space-y-0.5">
                      {countdown !== "-" && (
                        <div className={`${resetPrimary} text-[var(--color-accent)] font-medium`}>
                          in {countdown}
                        </div>
                      )}
                      {resetDisplay && (
